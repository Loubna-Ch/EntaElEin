...
export class UserRepository {
    ...
    static async create(userData: { 
        ...
        const { username, email, password, phonenumber, address, dateofbirth, role, regionid } = userData;
        ...
        const result = await pool.query(
            ...
            [username, email, password, phonenumber || null, address || null, dateofbirth || null, role || null, regionid || null]
        );
        return result.rows[0];
    }

    static async update(id: number, data: { username: string; email: string; phonenumber?: string; address?: string; dateofbirth?: string; role?: string }) {
        ...
        const result = await pool.query(
            `UPDATE public."User" 
             ...
             WHERE userid = $7
             RETURNING userid, username, email, role`,
            [username, email, phonenumber || null, address || null, dateofbirth || null, role || null, id]
        );
        return result.rows[0] || null;
    }
