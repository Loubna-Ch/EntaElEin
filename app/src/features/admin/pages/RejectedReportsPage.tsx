...
export function RejectedReportsPage() {
  const { token } = useAuth();
  const dispatch = useDispatch<AppDispatch>();
  const { items: reports, loading: reportsLoading, error: reportsError } = useSelector(
  ...
  useEffect(() => {
    ...
    const firstRejected = reports.find(
      (r: any) => String(r.status || "").toLowerCase() === "rejected"
    );
    if (firstRejected) {
      setSelectedReportId(Number(firstRejected.reportid ?? firstRejected.id));
    }
  ...
  const rejectedReports = useMemo(
    () => reports.filter((r) => String(r.status || "").toLowerCase() === "rejected"),
    [reports]
  );

  const selectedReport = rejectedReports.find((r: any) => Number(r.reportid ?? r.id) === Number(selectedReportId));

  const linkedParticipants = participants.filter((p: any) =>
    involvedInRows.some(
      (row: any) =>
        Number(row.reportid) === Number(selectedReportId) && Number(row.participantid) === Number(p.participantid)
    )
  );
  ...
  const handleCreateParticipant = async () => {
    ...
    try {
      ...
      setSuccess("Participant added to rejected report");
    } catch (err: any) {
  ...
  const handleUpdateParticipant = async () => {
    ...
    setError("");
    try {
      const updateRes = await fetch(`${API_URL}/participants/${editingParticipantId}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          ...(token ? { Authorization: `Bearer ${token}` } : {}),
  ...
  const updateRejectedReportStatus = async (nextStatus: "Pending" | "In Progress" | "Resolved") => {
    if (!selectedReport || !token) return;

    setParticipantSubmitting(true);
    setError("");
    try {
      const reportId = Number(selectedReport.reportid ?? selectedReport.id);
      const reportData = {
        crimedate: selectedReport.crimedate,
        description: selectedReport.description,
        status: nextStatus,
        image_url: selectedReport.image_url || undefined,
        userid: Number(selectedReport.userid),
        regionid: Number(selectedReport.regionid),
        hadasid: Number(selectedReport.hadasid),
      };

      const resultAction = await dispatch(updateReport({ reportId, reportData, token }));
      if (updateReport.rejected.match(resultAction)) {
        throw new Error((resultAction.payload as string) || "Failed to update report status");
      }

      await dispatch(fetchReports({ token: token || undefined }));
      setSelectedReportId(null);
      setSuccess(`Report moved to ${nextStatus}`);
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen bg-[#0f1729]">
      ...
      <div className="ml-64 px-12 py-8">
        <h1 className="text-3xl font-bold text-white">Rejected Reports</h1>
        <p className="text-[#94a3b8] mt-2">All reports marked as rejected</p>
        ...
        {reportsLoading ? (
          <div className="mt-8 text-[#94a3b8]">Loading rejected reports...</div>
        ) : (
          <div className="mt-8 grid grid-cols-1 xl:grid-cols-3 gap-4">
            {rejectedReports.length === 0 ? (
              <div className="text-[#94a3b8]">No rejected reports found.</div>
            ) : (
        ...
        {!reportsLoading && selectedReport && (
          <div className="mt-8 bg-[#1a2744] border border-white/10 rounded-xl p-6">
            ...
            <p className="text-[#94a3b8] mt-1">Review rejected reports and reopen them when new evidence is available.</p>

            <div className="mt-4 flex flex-wrap gap-2">
              <button
                onClick={() => updateRejectedReportStatus("Pending")}
                disabled={participantSubmitting}
                className="px-3 py-2 rounded-lg bg-amber-600 text-white text-sm font-semibold disabled:opacity-60"
              >
                Reopen As Pending
              </button>
              <button
                onClick={() => updateRejectedReportStatus("In Progress")}
                disabled={participantSubmitting}
              ...
              <button
                onClick={() => updateRejectedReportStatus("Resolved")}
                disabled={participantSubmitting}
