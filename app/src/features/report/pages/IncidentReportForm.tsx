...
const CrimeReport = () => {
	...
	const handleSubmit = async () => {
		...
		setLoading(true);
		try {
			const { reportdate, ...cleanData } = formData;
			const reportData = {
				...cleanData,
				status: "Pending",
			};

			const resultAction = await dispatch(createReport({ reportData, token }));
			
			if (createReport.fulfilled.match(resultAction)) {
				setSubmitted(true);
				toast.success("Report submitted successfully!", {
					description: "Your incident report has been securely transmitted.",
				});
				setTimeout(() => navigate('/dashboard/citizen'), 2500);
			} else {
				const errorMsg = resultAction.payload as string || "Submission failed";
				setError(errorMsg);
			...
			toast.error("Connection Error", { description: "Unable to connect to servers." });
		} finally {
			setLoading(false);
		}
	};
